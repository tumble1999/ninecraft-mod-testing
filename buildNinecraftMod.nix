{stdenv, writeText, androidenv,...}:
{pname?null,version?null,name?"${pname}-${version}",src,mainC?"main.c",...}:
let
  effectiveName = if name != null then name else
        if pname != null && version != null then "${pname}-${version}" else
          throw "Either 'name' must be provided, or both 'pname' and 'version'.";
